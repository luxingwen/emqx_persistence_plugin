...
-export([ on_message_publish/2, on_client_authenticate/2]).
-define(LOG(Level, Format, Args), emqx_logger:Level("emqx_persistence_plugin: " ++ Format, Args)).

register_metrics() ->
    [emqx_metrics:new(MetricName) || MetricName <- ['emqx_persistence_plugin.client_connected',
                                                    'emqx_persistence_plugin.authenticate',
...
%% Client authenticate
%%--------------------------------------------------------------------
on_client_authenticate(#{clientid := ClientId, username := Username, password := Password}, AuthResult) ->
    F = fun(R) -> case is_atom(R) of true -> atom_to_binary(R, utf8); _ -> R end end,
    case application:get_env(?APP, webauth, off) of
        true ->
...
on_client_connected(#{clientid := ClientId, username := Username, peerhost := {B1, B2, B3, B4}}, _ConnInfo, _Env) ->
    ...
    emqx_persistence_plugin_cli:insert(connect, [F(ClientId), F(Username), io_lib:format("~B.~B.~B.~B",[B1, B2, B3, B4])]),
    ok.
%%--------------------------------------------------------------------
%% Client disconnected
%%--------------------------------------------------------------------

on_client_disconnected(#{clientid := ClientId, username := Username}, Reason, #{peername := {{B1, B2, B3, B4}, Port}}, _Env) ->
    ...
    emqx_persistence_plugin_cli:insert(disconnect, [F2(ClientId), F2(Username), io_lib:format("~B.~B.~B.~B:~B",[B1, B2, B3, B4, Port]), F1(Reason)]),
    ok;
...
on_message_publish(Message = #message{id = <<I1:64, I2:48, I3:16>> = _MsgId, 
    ...
    ok = emqx_persistence_plugin_cli:insert(publish, [From, F2(Username), io_lib:format("~B.~B.~B.~B",[B1, B2, B3, B4]), I1 + I2 + I3, F1(Dup), F1(Retain), Qos, Topic, Payload, Ts]),
    {ok, Message};
% point to point
...
send_http_request(Params) ->
    ...
    Url = application:get_env(?APP, auth_url, "http://localhost:2500"),
